...
These methods used to estimate contributes from these processes are described in detail in Sections~\ref{sec:halo_estimate} and~\ref{sec:spike_estimate}, respectively. \PH{estimate contributes?=>rephrase}
...
Each simulation event has its weight multiplied by the value of the ratio evaluated at the number of true pileup interaction injected into the event. \PH{What is a simulation event}
...
\begin{figure}[htbp]
  ...
  \label{fig:pudist}
\end{figure}
\PH{This setion is completely nebulous, you don't actually expalin that you are deriving a weight. This needs a rewrite}

\section{Event Selection}
\label{sec:event_selection}

From the recorded data, events are selected by requiring $\met > 170\GeV$ and at least one photon with $\ETg > 175\GeV$ in the fiducial region of the ECAL barrel ($\abs{\eta} < 1.44$). Events with photons in the endcaps are not considered because the estimate of backgrounds due to beam halo and misidentified hadron are greatly complicated due to the $x$-$y$ grid of the crystals in the endcaps. \PH{This is very hard to follow can you put $x$-$y$ grid in layman's terms}
...
\PH{Only => ;only} Only jets with $\pt > 30\GeV$ and $\abs{\eta} < 5$ are considered in the \mindphijmet\ calculation.
...
\PH{The selection motivation could really benefit from plots so that we undestand the features.}
...
\PH{I don't understand this exactlytthe same comment. spelling : minizing }
...
\PH{What did you do for the MET cut uncertainty in the g+Jets. I honestly think you should at least split this MC out and quote a large unc. on it. }
