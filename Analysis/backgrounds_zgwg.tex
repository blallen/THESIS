...
\end{table}
\PH{Minor point. How was PS and merging dealt with in these samples. Strictly speaking MG is 2 parton but merged, so its slightly different}
...
\end{figure}
\PH{Its not completely obvious which unceratiny in the plot maps to the uncertainty in the text, please fix lables or add more to caption}
...
\PH{Please motivate the chioce of 590}
\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{
...
The only uncertainties in the transfer factor \RZll\ that do not largely cancel are those on lepton identification efficiency, the statistical uncertainty due to the limited MC sample size, and a minor uncertainty due to the different acceptances between the \zinvg\ and \zllg\ proccesses. \PH{spelling:proccesses} 
...
\PH{It would be nice to discuss the statistical limiations of htis process. How much would you gain if you had gamma events}
...
For the in-acceptance background ($|\eta| < 2.5$), the identification efficiency, which is lower for electrons than for muons, which translates to a larger background contribution from the electrons. \PH{To many whichs}
...
The overall result is a larger background contribution from events with out-of-acceptance muons than from events with out-of-acceptance electrons.  \PH{This discussion is lacking. You should probably discuss how hte MET calcualtion changes for muon in and out of acceptance when compared to electrons}
...
\PH{the logi is there, but this seems a little wonky. Its really the likelihood of a lepton to be out of acceptance varies with pT, so the failure rate grows with low pT.}
...
\begin{figure}[htbp]
  ...
  \label{fig:tf_w}
\end{figure}
\PH{The aption is inconsistent with the text}
...
\begin{figure}[htbp]
  ...
  \label{fig:tf_syst}
\end{figure}
\PH{Why are there two sets of uncertainties, there is just one going from W=>Z, the other direction is 100\% correlated, is this just that one set but applied in each sample?. Reading further the labels and text I think are wrong since you show the Z=>W ratio later}
