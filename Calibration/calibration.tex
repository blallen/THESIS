...
The ``tag-and-probe'' method described in Section~\ref{subsec:efake} with appropriate changes is used to measure the efficiency corresponding to the \egamma\ part of the photon ID in data. 

The first such change is that the sample is split into pass and fail categories depending on whether the probe passes the \egamma\ part of the photon ID with resulting effiency
\begin{equation*}
\epsilon_{\egamma} = \frac{N_{pass}}{N_{pass} + N_{fail}}.
\end{equation*}
The second such change is in the background model used in the TP fits. 
A data-driven template taken from a $\Pgm+\Pgg$ sample without any additional corrections is used in the following fits. 
As an alternative template to assess the systematic effect introduced by the choice of the background template, a simple linear function is also tested. 
