...
The electron and muon efficiencies are measured using a similar method to the ``tag-and-probe'' method described in Section~\ref{subsec:idsf}.
For these measurements, the tag object is an electron (muon) object passing the tight ID described in Section~\ref{sec:electrons} (Section~\ref{sec:muons}) and matched to a SingleElectron (SingleMuon) trigger while the probe object is an PF electron (muon) without any ID applied.
The passing (failing) categories are defined by events with probes passing (failing) the ID definition in question (see Sections~\ref{sec:electrons} and~\ref{sec:muons} for details).
