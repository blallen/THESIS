...
Now, we shall \PH{Missing Verb} the reconstruction of individual particles or physics objects from the electrical signals recorded by the subdetectors.
...
\PH{The above paragraph is quite cryptic. I struggled to figure out of the 5x1 is in $\eta$ or $\phi$. Also, how the iterative choice of the seeding works.  Given that this is an important part of your analysis can you rewrite.}
...
While not a physics object persay, isolation is a powerful variable used in the PF algorithm to distinguish prompt leptons and photons originating in the hard scattering from those originating in the decays of hardrons during the parton shower. \PH{persay is mispelled and also a colloquiusm, I don't think this should be in a thesis}
...
\begin{table}[htbp]
  ...
  \end{center}
\end{table}
\PH{All cut parameters on the electron id nee to be included. The point is that the thesis should allow me to reproduce the analysis completely}
...
\subsection{Hadrons} \PH{I would move this earlier, since the object ids occur after PF. Also you might want to label this PF or something closer to that. }
...
If the energy of the charged hadron exceeds its momentum by an amount larger than the calometric energy resolution, neutral hadrons and photons are added. \PH{This paragraph starts out of nowhere, was hard to follow reading it, perhaps merge with previous paragraph and rephrase a little}
...
\begin{equation}
  ...
  \end{matrix}
\end{equation}
respectively, where $q$ and $R$ are tunable parameters and $\dR_{ij}$ is the angular distance between the two particles. \PH{I do not recall a definition of $R$}
The distance parameter $R$ is an appromixate measure of the cone size \dR\ of the jet, while the power of the energy scale $q$ defines the relationship between the relationship between the momentum and angular factors. \PH{rephrase relationship between the...}
...
To remove muons from cosmic rays, muon candidates with trajectories more than 1\cm away from the beam axis are removed if the measured \met\ is consequently reduced by half. \PH{I don't think consequently is needed}
For muons with $\pt > 20\GeV$, the choice of subdetector used to estimate momemtum is reviewed and the smallest available estimate used if it reduces the measured \met\ by half. \PH{rephrase: the if it reduces confuseds me}
...
The gain-switch problem affected the analyses documented in this thesis, since large underestimation of the energy of a photon in an otherwise typical \gj\ event would introduce large missing transverse momentum to the event, typical collinear to the affected photon. \PH{grammar: typical collinear}
